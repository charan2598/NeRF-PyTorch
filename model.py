...
class NeRF_model(nn.Module):
    def __init__(
        self, position_encoding_length=10, direction_encoding_length=6, hidden_dim=256
    ):
        ...
        position_encoding_size = (
            position_size * embedding_size * self.position_encoding_length
        )
        direction_encoding_size = (
            direction_size * embedding_size * self.direction_encoding_length
        )
        ...
        self.block1 = nn.ModuleList(
            [
                nn.Linear(
                    in_features=position_encoding_size + position_size,
        ...
        self.block2 = nn.ModuleList(
            [
                nn.Linear(
                    in_features=hidden_dim + position_encoding_size + position_size,
                    out_features=hidden_dim,
                ),
                nn.Linear(in_features=hidden_dim, out_features=hidden_dim),
                nn.Linear(in_features=hidden_dim, out_features=hidden_dim),
        ...
        self.no_activation_layer = nn.Linear(
            in_features=hidden_dim, out_features=hidden_dim
        )

        self.density_layer = nn.Sequential(
            nn.Linear(in_features=hidden_dim, out_features=1), self.activation_layer
        )
        self.color_layer = nn.Sequential(
            nn.Linear(
                in_features=hidden_dim + direction_encoding_size + direction_size,
    ...
    def encoding_layer(values, encoding_length):
        ...
        for i in range(encoding_length):
            output = torch.cat(
                (output, torch.sin((2**i) * values), torch.cos((2**i) * values)), dim=1
            )
    ...
    def forward(self, positions, directions):
        ...
        positional_encoding = self.encoding_layer(
            positions, self.position_encoding_length
        )
        directional_encoding = self.encoding_layer(
            directions, self.direction_encoding_length
        )
        ...
        intermediate_output = torch.cat(
            (intermediate_output, positional_encoding), dim=1
        )
        ...
        colors = self.color_layer(
            torch.cat((intermediate_output, directional_encoding), dim=1)
        )
